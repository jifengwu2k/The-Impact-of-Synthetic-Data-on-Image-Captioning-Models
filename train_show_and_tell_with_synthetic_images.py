...
if __name__ == '__main__':
    ...
    parser.add_argument('--real-image-ratio', type=float, required=True)
    parser.add_argument('--synthetic-image-dir', type=str, required=True)
    parser.add_argument('--synthetic-image-ratio', type=float, required=True)
    ...
    real_image_ratio = args.real_image_ratio
    ...
    decoder = Decoder(embed_size=512, hidden_size=512, vocab_size=len(vocabulary), num_layers=3, stateful=False).to(device)
    ...
    for epoch in range(80):
        ...
        for i, (stacked_images_, stacked_captions_, caption_length_list_) in enumerate(pbar):
            ...
            features = encoder(stacked_images_on_device)
            outputs = decoder(features, stacked_captions_on_device, caption_length_list_)
